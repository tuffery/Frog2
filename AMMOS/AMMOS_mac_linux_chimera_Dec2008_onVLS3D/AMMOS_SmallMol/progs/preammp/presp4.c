...
int main()
{
char coordinate_input[200];
char ammp_output[200];
char dictionary[200];
...
FILE *in1,*dict,*output,*fopen();
	...
	fprintf(stderr," atom definition file:>\n");
/* strip out the file name and put it in the right place for opening */
        fgets( work,200,stdin );
        for(i= 0; i<200; i++)
                if( work[i] != ' ') break;
        for( ifile = i; ifile < 200 ; ifile++)
                {
                if( work[ifile] == ' ' ) break;
...
*/
	...
	fprintf(stderr," Residue dictionary directory:>\n");
/* strip out the file name and put it in the right place for opening */
        fgets( work,200,stdin );
        for(i= 0; i<200; i++)
                if( work[i] != ' ') break;
	...
	fprintf(stderr," atom input coordinate file:>\n");
/* strip out the file name and put it in the right place for opening */
        fgets( work,200,stdin );
        for(i= 0; i<200; i++)
                if( work[i] != ' ') break;
