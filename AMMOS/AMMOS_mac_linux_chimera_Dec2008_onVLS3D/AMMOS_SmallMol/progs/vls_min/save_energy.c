...
for (j = 0; j < max_numb_atoms; j++)
{
	...
	sscanf (buffer, "%s %s %f", word_1 [j], word_2 [j], &bef_total);
        ...
        fgets (buffer, line_length, before);
	/* Mod. P. Tuff 2010 */
        sscanf (buffer, "%f %s %s %s", &bef_nonbond, word_1 [j], word_2 [j], word_3 [j]);
        fgets (buffer, line_length, before);
        sscanf (buffer, "%f %s %s %s", &bef_total, word_1 [j], word_2 [j], word_3 [j]);
...
for (j = 0; j < max_numb_atoms; j++)
{
	...
	sscanf (buffer, "%s %s %f", word_1 [j], word_2 [j], &aft_total);
        ...
        fgets (buffer, line_length, after);
	/* Mod. P. Tuff 2010 */
        sscanf (buffer, "%f %s %s %s", &aft_nonbond, word_1 [j], word_2 [j], word_3 [j]);
        fgets (buffer, line_length, after);
        sscanf (buffer, "%f %s %s %s", &aft_total, word_1 [j], word_2 [j], word_3 [j]);

	delta_energy = aft_total - bef_total;


// Writting in final file

	fprintf (final, " %f : %f : %f : %f : %f : %f", 
		 bef_total, aft_total, delta_energy, bef_nonbond, aft_nonbond, aft_nonbond - bef_nonbond);
