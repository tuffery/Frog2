/* gauss.c
*
* collection of routines to service gauss screened  
*                      non-locaL nonbonded potentials
*
...
/* f_gauss()
*
* f_gauss increments the forces in the atom structures by the force
* due to the gaussed-nonbonded components.  NOTE THE WORD increment.
    ...
    for( i= 0; i< imax; i++)
    {
        ...
        for( ii=0; ii< jj;ii++)
        {
            ...
            */
            ...
            ra = r*alpha;
            k *= -two*pow(PI/alpha,2.5)*Fone(ra*0.5)*ROOTHALF*alpha*r0;
...
* V is the potential */
int v_gauss( V, lambda )
float *V,lambda;
{
    float r,r0,xt,yt,zt,ra;
    ...
    for( i= 0; i< imax; i++)
    {
        ...
        for( ii=0; ii< jj;ii++)
        {
            ...
            *  the econimized version
            	...
            	k *=  two*pow(PI/alpha,2.5)*Fzero(ra*0.5)*ROOTHALF;
            */
            k *= Fzero(r);
            r = r*r*r;
            k -= a1->a*a2->a/r;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int zone_gauss( V, lambda ,alist, inalist )
float *V,lambda;
ATOM *( *alist)[] ;
int inalist;
{
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_gauss( V, lambda,ilow,ihigh,op )
float *V,lambda;
int ilow,ihigh;
FILE *op;
{
