        ...
        for( i=0; i<= l; i++)
        {
            ...
            for( j=0; j< no; j++)
            {
                ...
                for( k=0; k <=j  ; k++)
                {
                    ...
                    total_col += z*x*cv*2.*(*gscoef)[l*no+i]*(*gscoef)[k*no+j];
                    if( k == j)
                    {
                        ...
                        y  = (*gscoef)[l*no+j]*(*gscoef)[i*no+k];
                        if( i != l)
                            y += (*gscoef)[i*no+j]*(*gscoef)[l*no+k];
                        /* this typo made the energy a lot better
                        	y = (*gscoef)[l*no+j]*(*gscoef)[i*no+k];
                        */
                        y += (*gscoef)[l*no+k]*(*gscoef)[i*no+j];
                        if( i != l)
...
float Cached_Ritz_orth( whatO, flag)
ORBITAL *whatO;
int flag;
{
    static float (*gscoef)[];
    static float (*o_cache)[];
    static float (*overlaps)[];
    static float (*twocent)[];
    static float (*fourcent)[];
    float H,ritzd,ritzn;
    int no,o_number();
    int i,j,k,l,m,n;
    int igrad;
    ...
    for( l=0; l<no; l++)
    {
        ...
        for( i=0; i<= l; i++)
        {
            ...
            if( i == l ) x = 1.;
#ifdef fast
            if( whatO->ncouple < 0 || whatO == o1 || whatO == o4 ||
                    whatO == o1->gang || whatO == o4->gang ){
#endif
                cv = orth_nuclear(o1,o4)+ orth_kinet(o1,o4);
#ifdef fast
            } else {
            ...
            for( j=0; j< no; j++)
            {
                ...
                for( k=0; k <=j  ; k++)
                {
                    ...
                    if( k == j) z = 1.;
#ifdef fast
                    if(whatO->ncouple < 0 ||  whatO == o1 || whatO == o2 ||
                            whatO == o3 || whatO == o4
                            || whatO == o1->gang || whatO == o2->gang ||
                            whatO == o3->gang || whatO == o4->gang ){
#endif
                        cv = orth_coloumb(o1,o2,o3,o4);
#ifdef fast
                    }else{
                    ...
                    if( j == k){
                        ...
                        total_ex -= cv*(*gscoef)[l*no+j]*(*gscoef)[i*no+k];
                        if( i != l )
                            total_ex -= cv*(*gscoef)[i*no+j]*(*gscoef)[l*no+k];
                    }
                    o3 = o3->next;
                }
                o2 = o2->next;
    ...
    H = total_nuc + total_enuc + total_kinet + total_col + total_ex;

    return H;
}/* end if Cached_Ritz_orth */
        ...
        *where[igrad] -= orth_orb_dstep;
        ...
        for( l=0; l<no; l++)
        {
            ...
            for( i=0; i<= l; i++)
            {
                ...
                if( i == l ) x = 1.;
#ifdef fast
                if( whatO[igrad]->ncouple < 0 || whatO[igrad] == o1 || whatO[igrad] == o4 ||
                ...
                for( j=0; j< no; j++)
                {
                    ...
                    for( k=0; k <=j  ; k++)
                    {
                        ...
                        if( k == j) z = 1.;
#ifdef fast
                        if( whatO[igrad]->ncouple < 0 || whatO[igrad] == o1 || whatO[igrad] == o2 ||
...
#ifdef fast
                if( whatO[igrad]->ncouple < 0 || o1 == whatO[igrad] || o4 == whatO[igrad]
