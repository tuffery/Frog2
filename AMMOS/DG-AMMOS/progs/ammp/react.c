/* react.c
*  damp charges as go out from the center of mass
*
*/
/* vnonbon.c
*
...
float lambda;
{
    ...
    */
    ...
    dddr = get_f_variable( "dddr");
    if( dddr < 1.e-7) dddr = 1.;
    dielectric = get_f_variable("dielec");
    if( dielectric < 1.) dielectric = 1.;
    if( dielecold != dielectric || dddr != dddrold)
    {
    ...
    if( vector == NULL)
    { aaerror("cannot allocate memory in v_react\n"); return 0;}
    /*
    ...
    */
    ...
    return 1;
DOIT:
    /*  cannot go to mmnonbon because don't have the code in there.
    	xt = get_f_variable("mmbox");
    	if( xt > 0.)
    	{ free(vector); free(atomall); mm_fv_update_react(lambda); return 1;}
    */
    radius = malloc( imax*sizeof(float));
    indexes = malloc( imax* sizeof(int) );
    if( indexes == NULL || radius == NULL ){
    ...
    {
        a1 = (*atomall)[ii];
        xt = (a1->x + lambda*a1->x -xcm);
        yt = (a1->y + lambda*a1->y -ycm);
        zt = (a1->z + lambda*a1->z -zcm);
        (*radius)[ii] =  dddr*sqrt(xt*xt + yt*yt+ zt*zt);
    }
...
#pragma _CNX no_recurrence
        ...
        for( i=ii+1; i< imax; i++)
        {
            ...
            {
                aaerror(
                    " fv_update_react> too many atoms increase NCLOSE or decrease mxcut");
            }
SKIPNEW:   i = i;
        }
        /* and then use them */
#pragma _CNX no_recurrence
        damp1 = one/ ( one + (*radius)[ii]);
        for( in=0; in< inindex; in++)
        {
            ...
            r = r0*r0;
            r = r*r*r; /* r0^-6 */
            damp2 = one / (one + (*radius)[i]);
            xt = a1->q*a2->q*dielectric*r0 *damp1*damp2;
            yt = a1->a*a2->a*r;
            zt = a1->b*a2->b*r*r;
            k = xt - yt + zt;
...
/* f_react()
*
* f_react increments the forces in the atom structures by the force
...
/*  returns 0 if error, 1 if OK */
{
    ...
    float (*buffer)[];
    int invector,atomsused,ii,jj,kk;
    float (*vector)[];
    ATOM *(*atms)[],*(*atomall)[];
    float dielectric;
    float fx,fy,fz;
    ...
    float yt2,yt3,yt4;
    float zt2,zt3,zt4;
    float dddr,(*radius)[],xcm,ycm,zcm,damp1,damp2;
    ...
    * forces, potentials and expansions thereof */
    ...
    i = a_number();
    atomall = malloc( i*sizeof(ATOM *) );
    radius = malloc( i*sizeof(float));
    if( atomall == NULL || radius == NULL)
    {aaerror("cannot allocate memory in f_react"); return 0;}
    ...
    for( i=0; i< imax; i++)
    {
        ...
        yt = (a1->y + lambda*a1->y -ycm);
        zt = (a1->z + lambda*a1->z -zcm);
        (*radius)[i] =  dddr*sqrt(xt*xt + yt*yt+ zt*zt);
    }
    ...
    for( i= 0; i< imax; i++)
    {
        ...
        for( ii=0; ii< jj;ii++)
        {
            ...
            k = -dielectric*a1->q*a2->q*r*damp1*damp2;
            r = r*r*r;
...
float *V,lambda;
{
    ...
    int invector,atomsused,ii,jj,imax,kk;
    float (*vector)[];
    float vx;
    float k2;
    ATOM *(*atomall)[];
    ...
    radius = malloc( i*sizeof(float) );
    if( atomall == NULL || radius == NULL)
    {aaerror("cannot allocate memory in v_react"); return 0;}
    ...
    for( i= 0; i< imax; i++)
    {
        ...
        for( ii=0; ii< jj;ii++)
        {
            ...
            /*      xt = xt/r0; yt = yt/r0; zt = zt/r0;
            */
            k = dielectric*a1->q*a2->q*r0*damp1*damp2;
            r = r*r*r;
            k -= a1->a*a2->a*r;
            k += a1->b*a2->b*r*r;
            vx += k;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int zone_react( V, lambda ,alist, inalist )
float *V,lambda;
ATOM *( *alist)[] ;
int inalist;
{
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_react( V, lambda,ilow,ihigh,op )
float *V,lambda;
int ilow,ihigh;
FILE *op;
{
    float r,r0,xt,yt,zt;
    float lcutoff,cutoff,get_f_variable();
    int inbond,inangle,i,ii,jj,numatm,a_number();
    ATOM *a1,*a2;
    ATOM *a_next( ); /* returns first ATOM when called with -1 */
    float dielectric,ve,va,vh;
    float vel,val,vhl;
    ...
    numatm = a_number();
    radius = malloc( numatm*sizeof(float));
    ...
    */
    ...
    /* unlike a_nonbon here we loop over everyone */
    a1 = a_next(-1);
    for( ii=0; ii< numatm; ii++)
    {
        if( a1->serial < ilow ) goto NOTANATOM;
        if( a1->serial > ihigh ) goto NOTANATOM;
        damp1 = one/ ( one + (*radius)[ii]);

        ve = 0.; va = 0.; vh = 0.;
        vel = 0.; val = 0.; vhl = 0.;
        for( jj=0; jj< numatm; jj++)
        {
            a2 = a_next(jj);
            /* goto SKIP is used because this is one case where it makes sense */
            for(i = 0; i< a1->dontuse; i++)
            ...
            ve += dielectric*a1->q*a2->q/r0*damp1*damp2;
            va -= a1->a*a2->a/r;
            vh += a1->b*a2->b/r/r;
            if( a2->serial < ilow || a2->serial > ihigh)
            {
                vel += dielectric*a1->q*a2->q/r0*damp1*damp2;
                val -= a1->a*a2->a/r;
                vhl += a1->b*a2->b/r/r;
            }
...
SKIP:
        ...
        fprintf(op,"Vreact internal %s %d Eq %f E6 %f E12 %f\n",
                a1->name,a1->serial,ve-vel,va-val,vh-vhl);
        fprintf(op,"Vreact external %s %d Eq %f E6 %f E12 %f\n",a1->name
                ,a1->serial,vel,val,vhl);
        fprintf(op,"Vreact total    %s %d Eq %f E6 %f E12 %f\n",a1->name
                ,a1->serial,ve,va,vh);
        *V += ve + va + vh;
...
NOTANATOM:
    ...
    fprintf(op," Vreact total internal %f \n",vtint);
    fprintf(op," Vreact total external %f \n",vtout);
    fprintf(op," Vreact total          %f \n",vtt);
    free (radius);
    return 1;
