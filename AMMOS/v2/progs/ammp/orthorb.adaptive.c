...
int ngrad;
{
    ...
    for( igrad = 0; igrad < ngrad; igrad++)
    {
        ...
        local_dstep = orth_orb_dstep;
NEW_STEP: ;

        for( i=0; i< ngrad; i++)
            *where[i] = params[i];

        *where[igrad] -= local_dstep;
        o_update_normals();
        renormalize();
        grad[igrad] -= nuclear();
        orth_invert( gscoef,overlaps,o_cache,no);
        ...
        *where[igrad] += local_dstep;
        ...
        /*	grad[igrad] *= 	-0.5e2;
        */
        printf("%f %f \n", local_dstep,grad[igrad]);
        if( grad[igrad] < orth_orb_dstep &&  grad[igrad]> -orth_orb_dstep)
        {
            local_dstep *= 10; goto NEW_STEP;
        }
        if( local_dstep != 0.)
            grad[igrad] *= -2.0/local_dstep;
        else
            grad[igrad] = 0.;
    }/* igrad */
