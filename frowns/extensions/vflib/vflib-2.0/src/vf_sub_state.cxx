 ...
 *
 * Author: P. Foggia
 * $Id: vf_sub_state.cxx,v 1.1 2003/03/19 20:58:53 wc2so1 Exp $
 *-----------------------------------------------------------------*/


/*-----------------------------------------------------------------
 * REVISION HISTORY
 *   $Log: vf_sub_state.cxx,v $
 *   Revision 1.1  2003/03/19 20:58:53  wc2so1
 ...
 *   Bug fix
 *
 *   Revision 1.2  1998/12/19 11:37:55  foggia
 *   Minor changes
 *
 *   Revision 1.1  1998/12/08 13:31:37  foggia
 *   Initial revision
 *
 *----------------------------------------------------------------*/
...
/*----------------------------------------------------------
 * Methods of the class VFSubState
 ---------------------------------------------------------*/

/*----------------------------------------------------------
 * VFSubState::VFSubState(g1, g2)
 * Constructor. Makes an empty state.
 ---------------------------------------------------------*/
VFSubState::VFSubState(Graph *ag1, Graph *ag2)
  { g1=ag1;
    g2=ag2;
    n1=g1->NodeCount();
    n2=g2->NodeCount();
...
/*----------------------------------------------------------
 * VFSubState::VFSubState(state)
 * Copy constructor. 
 ---------------------------------------------------------*/
VFSubState::VFSubState(const VFSubState &state)
  { g1=state.g1;
    g2=state.g2;
    n1=state.n1;
    n2=state.n2;
...
/*--------------------------------------------------------------------------
 * bool VFSubState::NextPair(pn1, pn2, prev_n1, prev_n2)
 ...
 * Returns false if no more pairs are available.
 -------------------------------------------------------------------------*/
bool VFSubState::NextPair(node_id *pn1, node_id *pn2,
  ...
  { byte cond1=0, cond2=0;
    ...
    while (prev_n2<n2 &&
               (node_flags_2[prev_n2] & cond1)!=cond2)
          prev_n2++;
...
/*---------------------------------------------------------------
 * bool VFSubState::IsFeasiblePair(node1, node2)
 ...
 *   criterion is not symmetric.
 --------------------------------------------------------------*/
bool VFSubState::IsFeasiblePair(node_id node1, node_id node2)
  { assert(node1<n1);
    assert(node2<n2);
...
/*--------------------------------------------------------------
 * void VFSubState::AddPair(node1, node2)
 * Adds a pair to the Core set of the state.
 * Precondition: the pair must be feasible
 -------------------------------------------------------------*/
void VFSubState::AddPair(node_id node1, node_id node2)
  { assert(node1<n1);
    assert(node2<n2);
    assert(core_len<n1);
    assert(core_len<n2);
...
/*----------------------------------------------------------------
 * Clones a VFSubState, allocating with new the clone.
 --------------------------------------------------------------*/
State* VFSubState::Clone()
  { return new VFSubState(*this);
  }
